...
调样式用 `tools/phone-preview.html`（要起本地静态服务器，访问 `/tools/phone-preview.html`）：左上角面板能切装哪个 app、停靠位、信号、状态栏时间，以及各 app 自己的参数（vlog 数据页那档是第几天 / 理智档位 / 素材是否出错，聊天那档是选当天的哪一段对话），不用真的玩到那一步。页面中间铺了一块按真事件窗尺寸做的假弹窗（最宽 560px、居中），把浏览器窗口拉窄就能看出角落姿态从哪个宽度开始遮挡。
